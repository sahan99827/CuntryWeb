   document.getElementById("fetchButton").addEventListener("click", getCountryData);
        ...
        function loadCities(countryName, container) {
            ...
            citiesDiv.innerHTML = '<h3>🏙️ Major Cities</h3><div class="loading" style="color: #667eea;">Loading cities...</div>';
            ...
            .then(data => {
                if (data.error || !data.data || data.data.length === 0) {
                    citiesDiv.innerHTML = '<h3>🏙️ Major Cities</h3><p style="color: #666;">No city data available</p>';
                    return;
                }
                ...
                citiesDiv.innerHTML = '<h3>🏙️ Major Cities</h3>';
                citiesDiv.appendChild(citiesGrid);
            })
            .catch(err => {
                citiesDiv.innerHTML = '<h3>🏙️ Major Cities</h3><p style="color: #666;">Could not load cities</p>';
            });
        }

        function createImagesSection(country) {
            let html = '<div class="images-section"><h3>🖼️ Country Images</h3><div class="image-grid">';
        ...
        function categorizeData(country) {
            return {
                "📋 Basic Information": {
                    ...
                    "Native Names": country.name?.nativeName ? Object.values(country.name.nativeName).map(n => n.common).join(", ") : "N/A",
                ...
                "📞 Contact & Communication": {
                    ...
                    "Calling Code": country.idd?.root + (country.idd?.suffixes?.join(", ") || ""),
        ...
        function filterCard(cardId, searchTerm) {
            ...
            const items = card.querySelectorAll(".info-item, .category-section, .cities-section");
